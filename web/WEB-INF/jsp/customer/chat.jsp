        ...
        <div class="w-1/3 border-r border-slate-200 bg-slate-50 flex flex-col hidden md:flex">
            <div class="p-4 border-b border-slate-200 bg-white">
                <h2 class="text-xl font-bold text-slate-800 flex items-center gap-2">
                    <span class="material-symbols-outlined text-[#4d661c]">chat</span> Trò chuyện
                </h2>
            </div>
                ...
                <c:choose>
                    <c:when test="${empty chatSessions}">
                        <div class="p-8 text-center text-slate-400">
                            ...
                            <p class="text-sm">Bạn chưa có cuộc trò chuyện nào.</p>
                        </div>
                    </c:when>
                    <c:otherwise>
                        <c:forEach var="session" items="${chatSessions}">
                            <a href="${pageContext.request.contextPath}/chat?shopId=${session.ownerId}" 
                               class="flex items-center gap-4 p-4 border-b border-slate-100 transition-colors ${session.sessionId == activeSessionId ? 'bg-emerald-50 border-emerald-100' : 'hover:bg-slate-100 bg-white'}">
                                <div class="w-12 h-12 rounded-full bg-slate-200 flex items-center justify-center shrink-0 border border-slate-300">
                                    <span class="material-symbols-outlined text-slate-500">storefront</span>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex justify-between items-center mb-1">
                                        <h3 class="font-bold text-slate-800 truncate">Shop #${session.ownerId}</h3>
            ...
            <c:choose>
                <c:when test="${activeSessionId > 0}">
                    ...
                    <div class="p-4 bg-white border-b border-slate-200 flex items-center justify-between shadow-sm z-10">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="font-bold text-slate-800">Trò chuyện với cửa hàng</h3>
                    ...
                    <div class="p-4 bg-white border-t border-slate-200">
                        <form id="chatForm" class="flex gap-2">
                            <input type="text" id="chatInput" autocomplete="off" placeholder="Nhập tin nhắn..." 
                ...
                <c:otherwise>
                    <div class="flex-1 flex flex-col items-center justify-center text-slate-400">
                        ...
                        <p class="text-sm">Chọn một đoạn chat ở danh sách bên trái để bắt đầu</p>
                    </div>
                </c:otherwise>
            </c:choose>
        </div>
...
<script>
    ...
    function formatTime(dateStr) {
        if (!dateStr) return '';
        // handle array [2026, 6, 5, 10, 30] from JavaTimeModule
        let date;
        ...
        fetch('${pageContext.request.contextPath}/api/chat?action=getMessages&sessionId=' + sessionId)
            ...
            .then(data => {
                if (data.success && data.messages.length > lastMessageCount) {
                    // Only append new messages to avoid flickering, or just re-render all if simple
