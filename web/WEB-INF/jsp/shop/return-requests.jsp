                    ...
                    <main class="flex-1 p-6 md:p-8 overflow-y-auto animate-fade-in-up opacity-0">
                        ...
                        <div class="glass-card rounded-2xl overflow-hidden mb-8">
                            <div class="p-6 border-b border-[#e2ece7] bg-[#f9fdf9] flex justify-between items-center">
                                ...
                                <span
                                    class="text-xs text-txt-2 bg-primary-lt px-3 py-1 rounded-full border border-primary/10">
                                            ...
                                            <c:forEach var="req" items="${returnRequests}">
                                                <tr>
                                                    <td class="text-center font-bold text-primary">#
                                                        <c:out value="${req.returnRequestId}" />
                                                    </td>
                                                    <td>
                                                        <a href="${pageContext.request.contextPath}/shop/orders?action=detail&orderId=${req.orderId}"
                                                    ...
                                                    <td>
                                                        <c:choose>
                                                            <c:when test="${not empty req.productName}">
                                                                <div class="font-bold text-txt">
                                                                    <c:out value="${req.productName}" />
                                                                </div>
                                                                <c:if test="${not empty req.variantLabel}">
                                                                    <span
                                                                        class="inline-block mt-0.5 text-[11px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded">
                                                                        Phân loại:
                                                                        <c:out value="${req.variantLabel}" />
                                                                    </span>
                                                                </c:if>
                                                            </c:when>
                                                            <c:otherwise>
                                                                <span class="text-txt-3 italic text-xs">Toàn bộ đơn
                                                                    hàng</span>
                                                            </c:otherwise>
                                                        </c:choose>
                                                    </td>
                                                    <td>
                                                        ...
                                                        <div class="font-semibold text-txt">#
                                                            <c:out value="${req.customerId}" />
                                                        </div>
                                                    </td>
                                                    <td class="text-center">
                                                        <span
                                                            class="inline-block px-2.5 py-1 rounded-full text-xs font-bold ${req.requestType == 'RETURN' ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-blue-50 text-blue-700 border border-blue-200'}">
                                                            <c:out value="${req.requestType}" />
                                                        </span>
                                                    </td>
                                                    <td>
                                                        <div class="font-bold text-txt text-xs">
                                                            <c:out value="${req.reasonCode}" />
                                                        </div>
                                                        <c:if test="${not empty req.description}">
                                                            <div
                                                                class="text-txt-2 text-xs mt-1 bg-slate-50 p-2 rounded border border-slate-100">
                                                                <c:out value="${req.description}" />
                                                            </div>
                                                        </c:if>
                                                        <c:if test="${not empty req.evidenceUrl}">
                                                            <div class="mt-1.5">
                                                                <a href="<c:out value=" ${req.evidenceUrl}" />"
                                                                target="_blank" class="inline-flex items-center gap-1
                                                                text-[11px] text-primary hover:underline font-bold
                                                                bg-[#edf7f2] px-2 py-1 rounded border border-[#bbf7d0]">
                                                                <i class="fa-solid fa-image"></i> Xem minh chứng
                                                                </a>
                                                            </div>
                                                        </c:if>
                                                    </td>
                                                    <td class="text-center font-bold text-txt">
                                                        <c:out value="${req.requestedQuantity}" />
                                                    </td>
                                                    <td class="text-right font-extrabold text-red-600">
                                                        <c:out value="${req.refundAmount}" />đ
                                                    </td>
                                                    <td class="text-center">
                                                        <c:choose>
                                                            <c:when test="${req.status == 'REQUESTED'}">
                                                                <span
                                                                    class="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-amber-50 text-amber-700 border border-amber-200">Đang
                                                                    chờ</span>
                                                            </c:when>
                                                            <c:when test="${req.status == 'APPROVED'}">
                                                                <span
                                                                    class="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">Đã
                                                                    nhận</span>
                                                            </c:when>
                                                            <c:otherwise>
                                                                <span
                                                                    class="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-rose-50 text-rose-700 border border-rose-200">Từ
                                                                    chối</span>
                                                            </c:otherwise>
                                                        </c:choose>
                                                    </td>
                                                    <td class="text-xs text-txt-2 font-medium">
                                                    ...
                                                    <td>
                                                        <c:choose>
                                                            <c:when test="${req.status == 'REQUESTED'}">
                                                                <span
                                                                    class="text-amber-600 font-bold text-xs italic flex items-center gap-1">
                                                                    <i class="fa-solid fa-spinner fa-spin"></i> Đang chờ
                                                                    xử lý...
                                                                </span>
                                                            </c:when>
                                                            <c:otherwise>
                                                                <div class="text-xs font-bold text-txt">
                                                                    ${req.status == 'APPROVED' ? 'Chấp nhận hoàn tiền' :
                                                                    'Từ chối hoàn trả'}
                                                                </div>
                                                                <c:if test="${not empty req.decisionReason}">
                                                                    <div class="text-[11px] text-txt-2 italic mt-0.5">Lý
                                                                        do:
                                                                        <c:out value="${req.decisionReason}" />
                                                                    </div>
                                                                </c:if>
                                                            </c:otherwise>
                                                        </c:choose>
