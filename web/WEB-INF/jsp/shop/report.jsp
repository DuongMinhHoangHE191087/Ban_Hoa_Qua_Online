            ...
            <body class="antialiased text-[#0f172a]">
                <div class="flex min-h-screen">
                    ...
                    <main class="flex-grow p-6 md:p-8 overflow-y-auto">
                        <!-- Header Section -->
                        <div
                            class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-[#f0faf3] via-[#e6f4ea] to-[#dcfce7] border border-[#bbf7d0]/80 p-6 rounded-3xl shadow-sm mb-8">
                            <div>
                                <h1 class="text-xl md:text-2xl font-extrabold text-[#364e03] tracking-tight font-sans">
                                    Thống Kê & Báo Cáo Doanh Thu</h1>
                                <p class="text-[#475569] text-xs md:text-sm mt-1">Phân tích chuyên sâu về doanh thu cửa
                                    hàng, tình trạng đơn hàng và hành vi mua sắm.</p>
                            </div>
                            <div class="flex items-center gap-2">
                                <button onclick="exportToCSV()"
                                    class="bg-[#4d661c] hover:bg-[#364e03] text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow-sm transition-all flex items-center gap-2 cursor-pointer border-none">
                        ...
                        <%-- Date Range Filter Form --%>
                            <form method="GET" action="${pageContext.request.contextPath}/shop/reports"
                                class="glass-card p-5 mb-8 rounded-2xl">
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                                    <div class="w-full">
                                        <label class="block text-xs font-bold text-txt-2 uppercase mb-1.5">Từ
                                            ngày</label>
                                        <div class="relative">
                                    ...
                                    <div class="w-full">
                                        <label class="block text-xs font-bold text-txt-2 uppercase mb-1.5">Đến
                                            ngày</label>
                                        <div class="relative">
                                    ...
                                    <div class="w-full">
                                        <label class="block text-xs font-bold text-txt-2 uppercase mb-1.5">Danh
                                            mục</label>
                                        <select name="categoryId"
                                            class="w-full bg-surface-2 border border-border px-3 py-2.5 rounded-xl text-txt text-sm focus:outline-none focus:border-[#4d661c]">
                                            <option value="">Tất cả danh mục</option>
                                            <c:forEach var="cat" items="${categories}">
                                                <option value="${cat.categoryId}" ${selectedCategoryId==cat.categoryId
                                                    ? 'selected' : '' }>
                                    ...
                                    <div class="flex gap-2 w-full lg:col-span-1">
                                        <button type="submit"
                                            class="bg-[#4d661c] hover:bg-[#364e03] text-white text-sm font-bold px-4 py-2.5 rounded-xl shadow-sm transition-all cursor-pointer flex-1 border-none text-center">
                                            <i class="fa-solid fa-filter mr-1"></i> Lọc
                                        </button>
                                        <button type="button" onclick="setQuickDateRange(30)"
                                            class="bg-white border border-border hover:bg-surface-2 text-txt text-xs px-2 py-2.5 rounded-xl shadow-sm transition-all cursor-pointer">
                                            30d
                                        </button>
                                        <button type="button" onclick="setQuickDateRange(7)"
                                            class="bg-white border border-border hover:bg-surface-2 text-txt text-xs px-2 py-2.5 rounded-xl shadow-sm transition-all cursor-pointer">
                                            7d
                                        </button>
                                    </div>
                            ...
                            <%-- KPI Cards --%>
                                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6 mb-8">
                                    <%-- Total Revenue --%>
                                        <div
                                            class="glass-card p-5 flex items-center gap-4 hover:-translate-y-0.5 transition-all duration-200 rounded-2xl cursor-default bg-gradient-to-br from-white to-[#f0fbf6]">
                                            <div
                                            ...
                                            <div>
                                                <span
                                                    class="text-xs font-bold text-txt-3 uppercase tracking-wider block">Doanh
                                                    thu shop</span>
                                                <h3 class="text-base font-black text-[#364e03] mt-0.5">
                                                    <ft:currency value="${kpiTotalRevenue}" />
                                                </h3>
                                            </div>
                                        </div>
                                        ...
                                        <%-- Estimated Revenue --%>
                                            <div
                                                class="glass-card p-5 flex items-center gap-4 hover:-translate-y-0.5 transition-all duration-200 rounded-2xl cursor-default bg-gradient-to-br from-white to-[#fffbeb]">
                                                <div
                                                ...
                                                <div>
                                                    <span
                                                        class="text-xs font-bold text-txt-3 uppercase tracking-wider block">Doanh
                                                        thu tạm tính</span>
                                                    <h3 class="text-base font-black text-txt mt-0.5">
                                                        <ft:currency value="${kpiEstimatedRevenue}" />
                                                    </h3>
                                                </div>
                                            </div>
                                            ...
                                            <%-- Total Orders --%>
                                                <div
                                                    class="glass-card p-5 flex items-center gap-4 hover:-translate-y-0.5 transition-all duration-200 rounded-2xl cursor-default">
                                                    <div
                                                    ...
                                                    <div>
                                                        <span
                                                            class="text-xs font-bold text-txt-3 uppercase tracking-wider block">Tổng
                                                            đơn hàng</span>
                                                        <h3 class="text-base font-black text-txt mt-0.5">
                                                            ${kpiTotalOrders} đơn</h3>
                                                    </div>
                                                </div>
                                                ...
                                                <%-- Success Orders --%>
                                                    <div
                                                        ...
                                                        <div>
                                                            <span
                                                                class="text-xs font-bold text-txt-3 uppercase tracking-wider block">Đơn
                                                                thành công</span>
                                                            <h3 class="text-base font-black text-txt mt-0.5">
                                                                ${kpiSuccessfulOrders} đơn</h3>
                                                        </div>
                                                    </div>

                                                    <%-- Cancellation Rate --%>
                                                        <div
                                                            class="glass-card p-5 flex items-center gap-4 hover:-translate-y-0.5 transition-all duration-200 rounded-2xl cursor-default bg-gradient-to-br from-white to-[#fef2f2]">
                                                            <div
                                                            ...
                                                            <div>
                                                                <span
                                                                    class="text-xs font-bold text-txt-3 uppercase tracking-wider block">Tỷ
                                                                    lệ hủy đơn</span>
                                                                <h3 class="text-base font-black text-txt mt-0.5">
                                                                    ${kpiCancellationRate}%</h3>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <%-- Total Units Sold --%>
                                                            <div
                                                                class="glass-card p-5 flex items-center gap-4 hover:-translate-y-0.5 transition-all duration-200 rounded-2xl cursor-default">
                                                                <div
                                                                    class="w-12 h-12 rounded-2xl bg-purple-50 text-purple-600 flex items-center justify-center text-xl shadow-inner">
                                                                    <i class="fa-solid fa-basket-shopping"></i>
                                                                </div>
                                                                <div>
                                                                    <span
                                                                        class="text-xs font-bold text-txt-3 uppercase tracking-wider block">Sản
                                                                        lượng bán</span>
                                                                    <h3 class="text-base font-black text-txt mt-0.5">
                                                                        ${kpiTotalUnitsSold} đv</h3>
                                                                </div>
                                                            </div>
                                </div>
                                ...
                                <%-- Charts Grid --%>
                                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                                        <%-- Revenue Trend Chart --%>
                                            <div
                                                class="glass-card p-6 flex flex-col justify-between rounded-2xl lg:col-span-2">
                                                <div
                                                    class="flex items-center justify-between mb-4 border-b border-border pb-3">
                                                    <h3 class="text-base font-bold text-txt"><i
                                                            class="fa-solid fa-chart-line text-primary mr-1.5"></i>Biểu
                                                        đồ xu hướng doanh thu</h3>
                                            ...
                                            <%-- Order Status stats chart --%>
                                                <div class="glass-card p-6 flex flex-col justify-between rounded-2xl">
                                                    <div
                                                        class="flex items-center justify-between mb-4 border-b border-border pb-3">
                                                        <h3 class="text-base font-bold text-txt"><i
                                                                class="fa-solid fa-chart-pie text-blue-500 mr-1.5"></i>Tỉ
                                                            lệ đơn hàng theo trạng thái</h3>
                                                ...
                                                <%-- Cancellation Reasons Chart --%>
                                                    <div
                                                        class="glass-card p-6 flex flex-col justify-between rounded-2xl">
                                                        <div
                                                            class="flex items-center justify-between mb-4 border-b border-border pb-3">
                                                            <h3 class="text-base font-bold text-txt"><i
                                                                    class="fa-solid fa-circle-question text-red-500 mr-1.5"></i>Lý
                                                                do hủy đơn phổ biến</h3>
                                    ...
                                    <%-- Fruit Usage Report Table --%>
                                        <div class="glass-card p-6 mb-8 rounded-2xl">
                                            <div
                                                class="flex items-center justify-between mb-6 border-b border-border pb-4">
                                                <h3 class="text-base font-bold text-txt"><i
                                                        class="fa-solid fa-basket-shopping text-amber-500 mr-1.5"></i>Báo
                                                    cáo bán hàng & mức sử dụng trái cây cửa hàng</h3>
                                                <span class="text-xs text-txt-3 font-semibold">Bộ dữ liệu giai
                                                    đoạn</span>
                                            </div>
                                            ...
                                            <div class="table-container overflow-x-auto">
                                                <table class="w-full border-collapse text-left" id="fruitUsageTable">
                                                    <thead>
                                                        <tr
                                                            class="border-b border-border text-xs font-bold text-txt-3 uppercase tracking-wider">
                                                    ...
                                                    <tbody class="divide-y divide-border text-sm text-txt-2">
                                                        <c:forEach var="item" items="${fruitUsage}">
                                                            <tr class="hover:bg-surface-2 transition-colors">
                                                                <td class="py-3.5 pr-4 font-bold text-txt">
                                                                ...
                                                                <td
                                                                    class="py-3.5 px-4 text-center font-semibold text-txt">
                                                                    ${item.totalQuantity}</td>
                                                                <td
                                                                    class="py-3.5 px-4 text-right font-bold text-primary">
                                                                    <ft:currency value="${item.totalAmount}" />
                                                                </td>
                                                                <td class="py-3.5 px-4 text-center">${item.orderCount}
                                                                    đơn hàng</td>
                                                            </tr>
                                                        </c:forEach>
                                                        <c:if test="${empty fruitUsage}">
                                                            <tr>
                                                                <td colspan="5"
                                                                    class="py-8 text-center text-txt-3 italic">Không có
                                                                    dữ liệu bán hàng nào trong giai đoạn này.</td>
                                                            </tr>
                                                        </c:if>
                                                    </tbody>
                                                </table>
                ...
                <div id="chart-data" class="hidden" data-revenue-trend='<c:out value="${empty revenueTrendJson ? ' []' :
                    revenueTrendJson}" escapeXml="true" />'
                data-order-status='
                <c:out value="${empty orderStatusStatsJson ? '[]' : orderStatusStatsJson}" escapeXml="true" />'
                data-cancellation='
                <c:out value="${empty cancellationReasonStatsJson ? '[]' : cancellationReasonStatsJson}"
                    escapeXml="true" />'>
                </div>
