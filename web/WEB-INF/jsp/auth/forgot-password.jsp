<%@ page contentType="text/html;charset=UTF-8" pageEncoding="UTF-8" language="java" %>
    <%@ taglib prefix="c" uri="jakarta.tags.core" %>
        ...
        <head>
            ...
            <title>Quên mật khẩu - MetaFruit</title>
        ...
        <body class="bg-emerald-50 text-on-surface min-h-screen flex flex-col antialiased relative">
            ...
            <%-- Decorative Background --%>
                ...
                <%-- Header --%>
                            ...
                            <%-- Step Progress Indicator --%>
                                <div class="flex items-center justify-center gap-2 mb-8">
                                    <div class="flex items-center gap-1.5">
                                        <div
                                            class="w-8 h-8 rounded-full bg-primary text-white flex items-center justify-center text-xs font-bold step-active shadow-md">
                                            1</div>
                                        <span class="text-xs font-semibold text-primary hidden sm:block">Nhập
                                            email</span>
                                    </div>
                                    <div class="h-px w-8 bg-outline-variant/50"></div>
                                    <div class="flex items-center gap-1.5">
                                        <div
                                            class="w-8 h-8 rounded-full bg-outline-variant/40 text-outline flex items-center justify-center text-xs font-bold">
                                            2</div>
                                ...
                                <%-- Title --%>
                                    <div class="text-center mb-8">
                                        <div
                                            class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-emerald-100 mb-4 shadow-inner">
                                            <span
                                                class="material-symbols-outlined text-primary text-4xl">lock_reset</span>
                                        </div>
                                        <h1 class="text-2xl md:text-3xl font-bold text-primary mb-2">Quên mật khẩu?</h1>
                                        <p
                                            class="text-sm md:text-base text-on-surface-variant font-light leading-relaxed">
                                    ...
                                    <%-- Error Message --%>
                                        <c:if test="${not empty requestScope.errorMsg}">
                                            <div
                                                class="mb-6 p-4 bg-red-50 border-l-4 border-error text-red-800 rounded-r-lg flex items-center gap-3 shadow-sm">
                                        ...
                                        <%-- Flash Message --%>
                                            <c:if test="${not empty sessionScope.flashMsg}">
                                                ...
                                                <div
                                                    class="mb-6 p-4 ${isError ? 'bg-red-50 border-error text-red-800' : 'bg-green-50 border-primary text-green-800'} border-l-4 rounded-r-lg flex items-center gap-3 shadow-sm">
                                                    <span
                                                        class="material-symbols-outlined ${isError ? 'text-error' : 'text-primary'}">${isError
                                                        ? 'error' : 'check_circle'}</span>
                                                    <span class="text-sm font-medium">
                                            ...
                                            <%-- Info note --%>
                                                <div
                                                    class="hidden mb-6 rounded-xl border border-emerald-200 bg-emerald-50/80 p-4 text-sm text-on-surface-variant flex items-start gap-2.5">
                                                    <span
                                                        class="material-symbols-outlined text-primary text-[18px] mt-0.5 flex-shrink-0">info</span>
                                                    <span>Nếu email chưa đăng ký, trang xác minh vẫn hiện ra nhưng
                                                        <strong>không gửi email</strong> — để bảo vệ thông tin tài
                                                        khoản.</span>
                                                </div>
                                                ...
                                                <%-- Form --%>
                                                        ...
                                                        <div
                                                            ...
                                                            <div class="flex flex-col gap-1">
                                                                ...
                                                                <div class="relative">
                                                                    ...
                                                                    <input
                                                                        class="w-full pl-9 pr-4 py-2.5 bg-white/70 border border-outline/30 focus:border-primary focus:ring-1 focus:ring-primary rounded-lg text-sm transition-all outline-none placeholder:text-outline-variant/60"
                                                                        id="email" name="email" value="<c:out value="
                                                                        ${param.email}" />"
                                                    ...
                                                    <%-- Footer link --%>
                                                        <div
                                                            class="mt-8 text-center border-t border-outline-variant/30 pt-6">
                    ...
                    <footer
                        class="w-full py-4 text-center border-t border-white/20 bg-white/40 backdrop-blur-md relative z-10">
                        <p class="text-xs text-on-surface-variant/80 font-light">&copy; 2026 MetaFruit. Giải pháp nông
                            sản Việt sạch và bền vững.</p>
                    </footer>
                    ...
                    <script>
                        ...
                        document.getElementById('forgotForm').addEventListener('submit', function () {
                            ...
                            btn.disabled = true;
                            btn.innerHTML = '<span class="material-symbols-outlined text-[18px] animate-spin">progress_activity</span><span>Đang gửi...</span>';
                        });
                    </script>
        </body>
