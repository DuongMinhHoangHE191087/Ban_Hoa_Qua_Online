/**
 * cart.js ? JavaScript qu?n lý trang Gi? hàng (Cart)
 * H? tr?: L?u Local Storage, Ajax m??t mà, Beacon API khi t?t tab, Check Stock khi Checkout.
 */
...
function getCartStockDisplayText(stockState) {
    ...
    if (stockState.overLimit || stockState.stock <= 10) {
        return `Ch? còn ${stockState.stock} s?n ph?m`;
    }
...
const CartPage = {
    isLoggedIn: false,
    contextPath: '',
    userCartKey: 'userCart', // Key l?u gi? hàng c?a user ?ã ??ng nh?p ? Local Storage
    ...
    async safeParseJSON(response) {
        ...
        if (!response.ok || !contentType || contentType.indexOf("application/json") === -1) {
            if (contentType && contentType.indexOf("application/json") !== -1) {
                ...
                throw new Error(errData.error || errData.message || `L?i h? th?ng (Mã: ${response.status})`);
            }
            throw new Error(`L?i h? th?ng (Mã: ${response.status})`);
        }
    ...
    init(isLoggedIn, contextPath) {
        ...
        // 1. N?u có guestCart khi v?a ??ng nh?p -> th?c hi?n ??ng b? g?p vào DB
        if (this.isLoggedIn) {
            this.syncGuestCartOnLogin();
        }

        // 2. Render t?c thì t? Local Storage ?? t?o ph?n h?i nhanh (0ms)
        this.renderImmediate();

        // 3. G?i Ajax ng?m l?y d? li?u gi? hàng chu?n t? Server (n?u ?ã ??ng nh?p)
        this.loadAndSyncFromServer();

        // 4. ??ng ký s? ki?n t?t tab / ?óng trình duy?t ?? ??ng b? DB (Beacon API)
        this.registerUnloadSync();

        // 5. ??ng ký các s? ki?n t??ng tác UI
        this.registerEvents();
    },

    /**
     * ??ng b? g?p gi? hàng khách vãng lai lên server khi ??ng nh?p thành công.
     */
    async syncGuestCartOnLogin() {
        ...
        try {
            ...
            if (data.success) {
                // Xóa gi? hàng guest vì ?ã g?p thành công
                GuestCart.clear();
                // L?u gi? hàng user ?ã g?p vào Local Storage
    ...
    /**
     * L?u gi? hàng user vào Local Storage
     */
    ...
    /**
     * L?u danh sách variant ?ang ???c ch?n ?? gi? state qua các l?n re-render.
     */
    saveSelectedVariantIds(variantIds) {
        try {
            ...
            // Không ch?n lu?ng checkout n?u localStorage không ghi ???c.
        }
    },

    /**
     * ??c danh sách variant ?ang ???c ch?n t? localStorage.
     */
    ...
    /**
     * L?y danh sách variant ?ang ???c tick trên DOM hi?n t?i.
     */
    ...
    /**
     * Khôi ph?c checkbox theo state ?ã l?u tr??c ?ó.
     * N?u ch?a có state nào, m?c ??nh ch?n toàn b? nh? hành vi c?.
     */
    ...
    /**
     * ??c gi? hàng user t? Local Storage
     */
    ...
    /**
     * L?y toàn b? items gi? hàng hi?n t?i tùy tr?ng thái ??ng nh?p
     */
    ...
    isStaleCartItemError(data) {
        ...
        return metaErrorCode === 'cart_item_not_found'
            || errorText.includes('Không tìm th?y s?n ph?m này trong gi? hàng')
            || errorText.includes('S?n ph?m không thu?c gi? hàng c?a b?n');
    },
    ...
    updateCheckoutButtonState(items) {
        ...
        btnCheckout.title = hasBlockingStockIssues
            ? 'Vui lòng x? lý các s?n ph?m h?t hàng tr??c khi thanh toán.'
            : (checkedVariantIds.length === 0
                ? 'Vui lòng ch?n ít nh?t m?t s?n ph?m ?? thanh toán.'
                : '');
    },

    /**
     * Render gi? hàng t?c thì t? Local Storage (không ch? server)
     */
    renderImmediate() {
        ...
        // T?o c?u trúc CartSummary gi? ??nh t? client ?? render tr??c
        let totalCents = 0;
        let totalGrams = 0;
    ...
    /**
     * G?i Ajax l?y gi? hàng chu?n t? DB (n?u ?ã ??ng nh?p) và c?p nh?t
     */
    async loadAndSyncFromServer() {
        ...
        try {
            ...
            if (payload && payload.cartSummary) {
                // L?u b?n chu?n vào Local Storage
                this.saveUserCartToLocal(payload.cartSummary.items);
                // Render l?i UI v?i d? li?u chu?n xác nh?t t? DB (n?u có bi?n ??ng giá/t?n kho)
                this.renderCart(payload.cartSummary);
            }
        } catch (err) {
            console.warn('[CartPage] Không th? k?t n?i server ?? ??ng b? gi? hàng:', err);
        }
    },

    /**
     * ??ng ký s? ki?n t?t tab ?? ??ng b? database qua Beacon API
     */
    registerUnloadSync() {
    ...
    /**
     * ??ng ký các s? ki?n click, nút t?ng gi?m s? l??ng, nút xóa, ??i bi?n th? và checkbox
     */
    registerEvents() {
        const cartContainer = document.getElementById('cart-items-container');
        if (!cartContainer) return;

        // S? ki?n t?ng gi?m s? l??ng, xóa
        cartContainer.addEventListener('click', (e) => {
            const target = e.target;
            
            // Nút gi?m s? l??ng (dùng closest ?? nh?n di?n chính xác k? c? click vào icon)
            ...
            if (target.closest('.btn-qty-plus')) {
                ...
                if (maxStock <= 0) {
                    this.showToast('S?n ph?m này hi?n ?ã h?t hàng. Vui lòng xóa kh?i gi? hàng ho?c ch?n phân lo?i khác.', 'warning');
                    ...
                    this.showToast(`Ch? còn t?i ?a ${maxStock} s?n ph?m trong kho!`, 'warning');
                }
            }
        ...
        // B?t s? ki?n gõ/thay ??i s? l??ng tr?c ti?p
        cartContainer.addEventListener('change', (e) => {
            const target = e.target;
        ...
        cartContainer.addEventListener('change', async (e) => {
            ...
            if (target.classList.contains('cart-variant-select')) {
                ...
                if (!newVariant) {
                    this.showToast('Không tìm th?y thông tin phân lo?i m?i.', 'error');
                    target.value = oldVariantId;
                    return;
                }

                if (newVariant.stockQuantity <= 0) {
                    this.showToast('R?t ti?c! Phân lo?i này hi?n ?ã h?t hàng.', 'warning');
                    target.value = oldVariantId;
                    return;
                }
                ...
                if (qty > newVariant.stockQuantity) {
                    ...
                    this.showToast(`S? l??ng ???c t? ??ng gi?m xu?ng ${qty} ?? phù h?p v?i kho hàng.`, 'warning');
                }
                ...
                if (this.isLoggedIn) {
                    try {
                        ...
                        if (data.success) {
                            ...
                            this.showToast('?ã ??i phân lo?i thành công!', 'success');
                            this.loadAndSyncFromServer();
                        } else {
                            if (!this.handleStaleCartItemError(data)) {
                                this.showToast(data.error || 'L?i ??i phân lo?i.', 'error');
                            }
                            target.value = oldVariantId;
                        }
                    } catch (err) {
                        this.showToast('L?i m?ng. Vui lòng th? l?i.', 'error');
                        target.value = oldVariantId;
                    }
                } else {
    ...
    /**
     * X? lý khi t?ng/gi?m s? l??ng s?n ph?m
     */
    async handleQuantityChange(element, quantity) {
        ...
        // C?p nh?t Local Storage t?c thì
        let localItems = this.getCurrentLocalItems();
        if (this.isLoggedIn) {
        ...
        // T? ??ng tính toán l?i Cart Summary ? Client ?? m??t mà l?p t?c
        this.recalculateClientSummary(localItems);

        // G?i Ajax c?p nh?t lên database (n?u ?ã ??ng nh?p)
        if (this.isLoggedIn) {
            try {
                ...
                if (!data.success) {
                    ...
                    if (errorCode === 'out_of_stock') {
                        this.showToast(data.error || 'S?n ph?m ?ã thay ??i t?n kho.', 'error');
                        // Khôi ph?c l?i s? l??ng chu?n t? server
                        this.loadAndSyncFromServer();
                    } else if (this.handleStaleCartItemError(data)) {
                        // Item ?ã b? ??i ID ho?c ??ng b? l?i t? server, refresh th?m l?ng thay vì b?n toast l?i.
                    } else {
                        this.showToast(data.error || 'Có l?i x?y ra', 'error');
                    }
                } else {
                    this.loadAndSyncFromServer();
                }
    ...
    /**
     * X? lý xóa s?n ph?m kh?i gi? hàng
     */
    async handleRemoveItem(row, cartItemId, variantId) {
        // Th?c hi?n micro-animation xóa ? client: m? d?n và co l?i tr??c khi bi?n m?t
        ...
        setTimeout(async () => {
            ...
            this.showToast('?ã xóa s?n ph?m kh?i gi? hàng.', 'success');

            // G?i Ajax xóa lên DB (n?u ?ã ??ng nh?p)
            if (this.isLoggedIn) {
                try {
                    ...
                    const data = await this.safeParseJSON(response);
                    if (!data.success) {
                        this.showToast(data.error || 'L?i khi xóa s?n ph?m trên server.', 'error');
                        this.loadAndSyncFromServer();
                    }
                } catch (err) {
                    console.warn('[CartPage] L?i k?t n?i xóa s?n ph?m:', err);
                }
            }
    ...
    /**
     * Tính toán l?i Summary ? phía client t?c thì ?? t?o c?m giác m??t mà
     */
    recalculateClientSummary(items) {
        ...
        if (txtCount) txtCount.textContent = `${checkedCount} m?t hàng`;
        if (txtCountTop) txtCountTop.textContent = `${items.length} m?t hàng`;
    ...
    /**
     * Render toàn b? gi? hàng lên HTML
     */
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (payload && payload.variants) {
                        ...
                        const sName = payload.product?.shopName || 'C?a hàng';
                ...
                console.warn(`[CartPage] Không th? l?y bi?n th? cho s?n ph?m ${productId}:`, err);
            }
        }));
    ...
    renderCart(summary) {
        ...
        summary.items.forEach(item => {
            const shopName = (item.shopName || "C?a hàng").trim();
        ...
        if (stockIssues.length > 0) {
            ...
            const statusLine = [
                outOfStockCount > 0 ? `${outOfStockCount} s?n ph?m ?ã h?t hàng` : null,
                ...
                <div class="mb-4 rounded-2xl border border-amber-200 bg-amber-50/90 text-amber-900 p-4 shadow-sm">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="text-sm leading-relaxed">
                            <p class="font-bold">Có ${stockIssues.length} s?n ph?m ch?a th? thanh toán ngay.</p>
                            <p class="mt-1">${statusLine || 'Vui lòng ki?m tra l?i s? l??ng và phân lo?i còn hàng.'} Hãy ??i sang phân lo?i còn t?n kho ho?c gi?m s? l??ng tr??c khi thanh toán.</p>
                        </div>
                    </div>
                </div>
            `;
                            ...
                            <div class="flex justify-between items-start w-full">
                                <div>
                                    ...
                                    <p class="font-body-md text-body-md text-on-surface-variant text-[11px] mt-1 text-muted">
                                        ?óng gói: <span class="bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded text-[11px] font-semibold border border-[#BBF7D0]/40">${item.packagingLabel} (+${CurrencyFmt.format(packagingPriceAdd)})</span>
                                    </p>
                                    ` : ''}
                                </div>
                                <div class="flex flex-col items-end">
                                    ...
                                    ${packagingPriceAdd > 0 ? `<span class="text-[10px] text-gray-400">G?m ?óng gói +${CurrencyFmt.format(packagingPriceAdd)}</span>` : ''}
                                </div>
                            </div>
                            
                            <div class="flex justify-between items-center mt-sm w-full">
                                <!-- B? Spinner s? l??ng bo tròn cao c?p -->
                                ...
                                <!-- Nút xóa s?n ph?m v?i micro-animation -->
        ...
        if (container) {
            container.innerHTML = `
                <div class="text-center py-12 premium-glass-card rounded-[1.5rem] p-lg">
                    <img src="${this.contextPath}/assets/images/empty_cart.png" alt="Gi? hàng tr?ng" class="mx-auto mb-6 opacity-70" style="max-width: 150px;">
                    <h3 class="font-headline-md text-headline-md text-inverse-surface font-bold text-dark text-xl">Gi? hàng c?a b?n ?ang tr?ng!</h3>
                    <p class="text-on-surface-variant mt-2 text-sm text-muted">Hãy quay l?i c?a hàng ?? l?a ch?n các lo?i hoa qu? t??i ngon nh?t nhé.</p>
    ...
    /**
     * X? lý nút thanh toán - Ki?m tra t?n kho DB th?c t? ch?ng xung ??t và g?i danh sách variantId ???c ch?n
     */
    async handleCheckout() {
        if (!this.isLoggedIn) {
            this.showToast('B?n c?n ??ng nh?p ?? ti?n hành thanh toán!', 'warning');
            setTimeout(() => {
        ...
        if (blockedRows.length > 0) {
            this.showToast('Có s?n ph?m ?ã h?t hàng ho?c v??t t?n kho. Hãy ??i sang phân lo?i còn hàng ho?c gi?m s? l??ng tr??c khi thanh toán.', 'warning');
            return;
        }

        const checkedVariantIds = this.getCheckedVariantIdsFromDom();
        if (checkedVariantIds.length === 0) {
            this.showToast('Vui lòng ch?n ít nh?t m?t s?n ph?m ?? thanh toán!', 'warning');
            return;
        }
                ...
                if (errors && errors.length > 0) {
                    ...
                    if (typeof Swal !== 'undefined') {
                        const intro = isOutOfSeason
                            ? 'M?t s? s?n ph?m trong gi? ?ã h?t mùa nên không th? thanh toán ngay lúc này.'
                            : 'M?t s? s?n ph?m trong gi? hàng ?ã thay ??i t?n kho th?c t?.';
                        Swal.fire({
                            icon: isOutOfSeason ? 'warning' : 'error',
                            title: isOutOfSeason ? 'S?n ph?m h?t mùa' : 'C?nh báo t?n kho',
                    ...
                    if (blockedRows.length > 0) {
                        this.showToast('Có s?n ph?m trong gi? ?ã h?t hàng ho?c v??t t?n kho. Hãy ??i phân lo?i còn hàng ho?c gi?m s? l??ng.', 'warning');
                    } else {
                        this.showToast(data.error || 'Không th? ki?m tra t?n kho lúc này.', 'error');
                    }
                }
            }
        } catch (err) {
            console.error('[CartPage] L?i m?ng khi ki?m tra t?n kho:', err);
            this.showToast('L?i k?t n?i m?ng. Không th? ki?m tra t?n kho lúc này.', 'error');
        } finally {
    ...
    /**
     * Hi?n th? popup modal thông báo l?i t?n kho chi ti?t
     */
    showStockErrorModal(errorContent) {
        // T?o modal dynamically n?u ch?a có
                ...
                <div id="stock-error-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm hidden opacity-0 transition-opacity duration-300">
                    <div class="bg-white premium-glass-card rounded-[1.5rem] max-w-md w-full overflow-hidden shadow-2xl scale-95 transition-transform duration-300 p-6 relative border border-slate-100">
                        ...
                        <div class="flex items-center gap-3 text-red-500 mb-4">
                            ...
                            <h3 class="font-headline-sm text-headline-sm font-bold text-lg text-rose-700">C?nh báo T?n kho</h3>
                        </div>
                        <div class="py-2 text-slate-800 text-sm">
                            <p class="font-semibold mb-3">R?t ti?c! M?t s? s?n ph?m trong gi? hàng ?ã thay ??i t?n kho th?c t? do có khách hàng khác v?a ??t mua:</p>
                            ...
                            <p class="mb-0 text-xs text-slate-500">H? th?ng ?ã t? ??ng c?p nh?t gi? hàng v? s? l??ng t?i ?a có th? mua. Vui lòng ki?m tra l?i gi? hàng tr??c khi nh?n Thanh toán.</p>
                        </div>
                        <div class="flex justify-end gap-3 mt-6">
                            <button type="button" class="bg-primary text-on-primary hover:bg-opacity-90 font-semibold px-6 py-2.5 rounded-full text-sm shadow-md transition-all duration-200" onclick="CartPage.hideStockErrorModal()">??ng ý</button>
                        </div>
                    </div>
                </div>
            `;
    ...
    /**
     * Hi?n th? Toast thông báo nhanh s? d?ng Tailwind CSS thu?n không ph? thu?c Bootstrap
     */
