...
import servlet.auth.LoginServlet;
...
public class LoginServletShopStatusRegressionTest {
    ...
    public void setUp() throws SQLException {
        ...
        customerId = userDAO.saveNewCustomer(
                "Login Status Customer",
                "login_status_" + System.currentTimeMillis() + "@test.com",
        ...
        profile.setShopName("Login Status Shop");
        ...
        profile.setBusinessEmail("login_shop_" + customerId + "@test.com");
    ...
    public void tearDown() {
        try {
            ...
            System.err.println("[LoginServletShopStatusRegressionTest] Cleanup failed: " + e.getMessage());
        }
    }

    @Test
    public void should_redirectToShopStatus_when_loggedInCustomerHasShopProfile() throws Exception {
        LoginServlet servlet = new LoginServlet();
        MockEnv env = new MockEnv(buildUser(customerId, AppConfig.ROLE_CUSTOMER));

        Method doGet = LoginServlet.class.getDeclaredMethod("doGet", HttpServletRequest.class, HttpServletResponse.class);
        doGet.setAccessible(true);
        doGet.invoke(servlet, env.request, env.response);

        assertEquals("/Ban_Hoa_Qua_Online/shop/status", env.redirectTarget.get());
        assertEquals(null, env.forwardPath.get());
        assertTrue("Session phải có flash success", env.sessionAttrs.containsKey(AppConfig.SESSION_FLASH_MSG));
    }
            ...
            request = (HttpServletRequest) Proxy.newProxyInstance(
                    ...
                    (proxy, method, args) -> {
                        switch (method.getName()) {
                            ...
                            case "getRequestURI":
                                return "/Ban_Hoa_Qua_Online/auth/login";
                            case "getParameter":
                                return null;
                            case "getAttribute":
