...
| **Description** | The checkout transaction. Validates the customer and recipient details, recalculates the quote server-side, applies coupons and delivery fees, reserves inventory, splits a multi-shop cart into one parent order plus one child order per shop, and creates the correct COD or bank-transfer payment state. |
| **Preconditions** | - PRE-1: The Customer is authenticated and allowed to access checkout.<br>- PRE-2: The cart contains at least one valid item.<br>- PRE-3: Product, variant, stock, pricing, delivery, and payment configuration are available. |
| **Trigger** | The Customer clicks the "Place Order" button. |
| **Postconditions** | - POST-1: Orders and order-item snapshots are persisted with recipient and pricing data.<br>- POST-2: Stock is reserved atomically and checked-out cart items are removed.<br>- POST-3: COD order: order status is `CONFIRMED`, with no online payment required.<br>- POST-4: Bank-transfer order: order status is `PENDING_PAYMENT`, a pending payment transaction is created, and the Customer is redirected to the QR payment page. |
| **Normal Flow** | 1. The Customer opens Checkout; the system validates authentication and selected cart items.<br>2. The Customer enters or selects recipient name, Vietnamese phone number, delivery address, delivery time slot, notes, coupons, and `COD` or `CK`.<br>3. The Customer submits the checkout form with a valid CSRF token.<br>4. The system locks the customer checkout request, rebuilds the quote from database data, and validates prices, active status, quantity, stock, weight, coupons, delivery fee, and final amount.<br>5. In one database transaction, the system locks/reserves inventory, creates order and item snapshots, allocates discounts, and splits items by shop when necessary.<br>6. For `COD`, the system validates COD eligibility and the below-2,000,000 VND limit, creates `CONFIRMED` order state, and returns the success page.<br>7. For `CK`, the system creates one pending payment transaction on the root order with exact final amount, SePay reference, and 10-minute expiry, then redirects to the payment page.<br>8. The system clears only the successfully checked-out cart items and shows the order/payment next step. |
| **Alternative Flows** | **4.1. Cart or stock changed**<br>1. The system rejects the quote and rolls back the transaction.<br>2. The cart remains available and the Customer is asked to review quantity, price, or availability.<br><br>**5.1. Multiple shops**<br>1. The system creates one `PARENT` root order and one `CHILD` order per shop.<br>2. The root order owns the bank-transfer payment transaction; child orders retain shop ownership and fulfillment data.<br><br>**6.1. COD selected**<br>1. If the amount is at least 2,000,000 VND or the customer is not COD-eligible, the system rejects COD and asks the Customer to select bank transfer. |
| **Exceptions** | **2.E1 Invalid recipient data**: The system blocks submission and shows the field-specific validation message.<br><br>**3.E1 Invalid CSRF or unauthorized customer**: The system rejects the request and does not create an order.<br><br>**4.E1 Invalid selection, inactive product, insufficient stock, excessive weight, invalid coupon, or changed price**: The transaction is rolled back; no partial order or reservation remains.<br><br>**5.E1 Duplicate submission/checkout lock**: The system rejects the concurrent request and asks the Customer not to click repeatedly.<br><br>**7.E1 Database failure**: The system rolls back order, item, inventory, promotion, and payment changes, then returns the Customer to checkout. |
| **Priority** | High (P0 - Key Transactional Path) |
| **Frequency of Use** | Extremely High. Synced on every checkout. |
| **Business Rules** | - INV-01: Inventory is reserved atomically for the checkout/payment lifecycle and released on cancellation or expiry.<br>- ORD-02: Quote and final amount are calculated server-side and cannot be negative.<br>- CHK-02: Multi-shop cart splits into one Parent and shop-owned Child orders.<br>- CHK-04: Discounts and fees are allocated consistently to child orders.<br>- CHK-06: Order creation is blocked when stock is insufficient.<br>- PAY-01: COD is available only below 2,000,000 VND and for COD-eligible customers. |
...
| **Description** | Completes the payment lifecycle after UC-07. COD is already confirmed during order placement; bank transfer uses a dynamic VietQR/SePay transaction, exact amount/reference matching, webhook reconciliation, and optional customer notification for manual admin review. |
| **Preconditions** | - PRE-1: A customer-owned order exists.<br>- PRE-2: For bank transfer, the root order is `PENDING_PAYMENT` and has a pending payment transaction.<br>- PRE-3: For COD, the order was created as `CONFIRMED` by UC-07. |
| **Trigger** | For bank transfer, the Customer opens the payment page, transfers the exact amount, or clicks "I have paid". For COD, the trigger is delivery completion/collection rather than an online payment action. |
| **Postconditions** | - POST-1: Successful bank transfer sets payment transaction to `completed` and root/child orders to `CONFIRMED`.<br>- POST-2: Duplicate, wrong-reference, wrong-direction, or wrong-amount webhook does not mark the order paid.<br>- POST-3: Expired or cancelled payment releases the reservation and leaves the order unpaid/cancelled according to the cancellation workflow.<br>- POST-4: COD remains `CONFIRMED` until delivery staff records cash collection. |
| **Normal Flow** | 1. For `CK`, the system displays bank, account, exact amount, SePay reference, QR, and a 10-minute expiry countdown.<br>2. The Customer transfers the exact amount with the displayed reference.<br>3. SePay sends a webhook; the system validates the payload, incoming transfer direction, reference/order ownership, exact amount, and deduplicates the provider transaction id.<br>4. In one transaction, the system marks the payment `completed`, confirms the root order and all child orders, and sends customer/shop notifications asynchronously.<br>5. If the webhook is delayed, the Customer may click "I have paid"; the transaction becomes `processing` for admin verification and is not falsely marked completed.<br>6. Admin reviews a processing payment and approves it only after verifying the bank evidence; the same finalization and notification flow runs.<br>7. The Customer can poll the status endpoint and is redirected to the success/order-detail page after confirmation. |
| **Alternative Flows** | **1.1. QR expired**: The Customer requests a renewal; the system creates a new 10-minute QR/payment expiry only while the order is still pending.<br><br>**2.1. Manual review**: If automatic matching is unavailable, admin verifies the bank transfer and approves the pending transaction once; duplicate approval is ignored safely.<br><br>**3.1. Duplicate webhook**: The system returns a handled duplicate result without completing the payment twice. |
| **Exceptions** | **3.E1 Invalid/missing webhook payload**: The system rejects it and keeps the order unpaid.<br><br>**3.E2 Wrong amount/reference/account/direction**: The system records the mismatch for review and does not confirm the order.<br><br>**4.E1 Payment transaction expired/cancelled**: The system does not mark the order paid; the expiry workflow cancels the order and releases stock when applicable.<br><br>**5.E1 Unauthorized customer order access**: The system does not disclose payment or order data.<br><br>**7.E1 Internal processing failure**: The database transaction rolls back so payment and order states cannot be partially finalized. |
| **Priority** | High (P0 - Key Revenue Gate) |
| **Frequency of Use** | Extremely High. Executed on every checkout. |
| **Business Rules** | - PAY-01: COD is limited to amounts below 2,000,000 VND and eligible customers.<br>- PAY-02: Bank-transfer QR/payment expiry is 10 minutes; renewal is allowed only for a still-pending order.<br>- PAY-03: Payment completion requires exact amount and valid SePay reference/provider transaction id.<br>- PAY-04: Webhook processing is idempotent and must not complete a payment twice.<br>- PAY-05: Payment completion confirms the root order and cascades to child orders; COD collection is recorded separately at delivery. |
